...
def l_plotting(train_data, test_data):
	...
	fig.suptitle('Final NMSE vs Learning Rate (lower range) for LMS Filter', fontsize=36)
	...
	for tick in ax.yaxis.get_major_ticks():
		tick.label.set_fontsize(20)
	#plt.show()
	plt.savefig('plots_lms/snrvslrl_code.png')
	with open('plots_lms/snrvslrl_code.pkl', "wb") as fp:
		pickle.dump(fig, fp, protocol=4)
	with open('plots_lms/snrvslrl_list.pkl', "wb") as fp:
...
def h_plotting(train_data, test_data):
	...
	start, end = ax.get_ylim()
	ax.yaxis.set_major_formatter(ticker.FormatStrFormatter('%0.1f'))
	fig.suptitle('Final NMSE vs Learning Rate (higher range) for LMS Filter', fontsize=36)
	plt.ylabel('NMSE', fontsize=24)
	plt.xlabel('Learning Rate', fontsize=24)
	for tick in ax.xaxis.get_major_ticks():
		tick.label.set_fontsize(20)
	for tick in ax.yaxis.get_major_ticks():
		tick.label.set_fontsize(20)
	#plt.show()
	plt.savefig('plots_lms/snrvslrh_code.png')
	with open('plots_lms/snrvslrh_code.pkl', "wb") as fp:
		pickle.dump(fig, fp, protocol=4)
	with open('plots_lms/snrvslrh_list.pkl', "wb") as fp:
		pickle.dump(train_data, fp)
		pickle.dump(test_data, fp)
...
def fl_plotting(train_data, test_data):
	...
	ax.yaxis.set_major_formatter(ticker.FormatStrFormatter('%0.1f'))
	fig.suptitle('Final NMSE vs Filter Length for LMS Filter', fontsize=36)
	plt.ylabel('NMSE', fontsize=24)
	...
	for tick in ax.yaxis.get_major_ticks():
		tick.label.set_fontsize(20)
	#plt.show()
	plt.savefig('plots_lms/snrvsfl_code.png')
	with open('plots_lms/snrvsfl_code.pkl', "wb") as fp:
		pickle.dump(fig, fp, protocol=4)
	with open('plots_lms/snrvsfl_list.pkl', "wb") as fp:
		pickle.dump(train_data, fp)
		pickle.dump(test_data, fp)
