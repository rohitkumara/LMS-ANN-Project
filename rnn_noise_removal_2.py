...
def get_data(filename):
	...
	bandlimit_white_noise = butter_lowpass(white_noise, 5000,Fs,order = 10)
	scaled_bandlimit_white_noise = (np.max(data)/np.max(bandlimit_white_noise))*bandlimit_white_noise

	bandlimit_pink_noise = butter_lowpass(pink_noise, 5000,Fs,order = 10)
	scaled_bandlimit_pink_noise = (np.max(data)/np.max(bandlimit_pink_noise))*bandlimit_pink_noise
	data_noise = data + np.abs(scaled_bandlimit_white_noise)
	#data_noise = data + np.abs(scaled_bandlimit_pink_noise) #Uncomment and comment the earlier line for pink noise
...
def main():
	...
	trainX_o,trainY_o, Fs = get_data('Mockingbird.wav')
	...
	trainX_o,trainY_o, Fs = get_data('Australian.wav')
	print('Playing noisy data')
	print(measure_snr(trainX_o,trainY_o))
	#play_file(trainX_o, Fs)
	print('Playing actual data')
