...
def measure_snr(noisy, noise):
	...
	print('Pwr Data: {} Pwr Noise: {}'.format(pwr_data, pwr_noise))
	snr = pwr_data/pwr_noise
	return 10*np.log10(snr)

def main():
	#data preprocessing step
	temp1, Fs = get_data('landing.wav')
	temp2_1, Fs = get_data('Mockingbird.wav')
	temp2_2, Fs = get_data('Australian.wav')
	data = np.concatenate((temp2_1,temp2_2))
	noise = temp1
	...
	model = Sequential()
	model.add(GRU(64, return_sequences = True, input_shape=(tap, 1)))
	...
	'''diff_err = np.abs(predict - trainY_o[tap-1:-1].reshape(predict.size,1))
		plt.plot(diff_err)
		plt.show()'''

	temp1, Fs = get_data('landing.wav')
	temp2, Fs = get_data('Nightingale.wav')
	start = time.time()
